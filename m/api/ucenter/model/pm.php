...
class pmmodel
{
    ...
    public function getpmlist($uid, $filter, $start, $ppp = 10)
    {
        ...
        if ($members) {
            ...
            foreach ($members as $key => $data) {
                ...
                if ($today <= $data['lastdateline']) {
                    $daterange = 1;
                } else {
                    if ($today - 86400 <= $data['lastdateline']) {
                        $daterange = 2;
                    } else {
                        if ($today - 172800 <= $data['lastdateline']) {
                            $daterange = 3;
                        } else {
                            if ($today - 604800 <= $data['lastdateline']) {
                                $daterange = 4;
                            }
                        }
                    }
    ...
    public function ispminterval($uid, $interval = 0)
    {
        ...
        if ($interval < $this->base->time - $lastupdate) {
            return 1;
        } else {
            return 0;
        }
