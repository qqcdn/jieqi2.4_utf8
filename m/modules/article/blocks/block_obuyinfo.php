...
class BlockArticleObuyinfo extends JieqiBlock
{
    public $module = 'article';
    public $template = 'block_obuyinfo.html';
    public $exevars = array('field' => 'obuyinfoid', 'listnum' => 10, 'asc' => 0, 'articleid' => '0');
    public function __construct(&$vars)
    {
        ...
        if (!empty($this->blockvars['vars'])) {
            ...
            if (0 < $arynum) {
                ...
                if (in_array($varary[0], array('obuyinfoid', 'articleid', 'obookid', 'userid', 'buytime'))) {
    ...
    public function setContent($isreturn = false)
    {
        ...
        $sql = 'SELECT * FROM ' . jieqi_dbprefix('obook_obuyinfo') . ' WHERE ' . $slimit . ' ORDER BY ' . $sort . ' ' . $order . ' LIMIT ' . $limitstart . ',' . $listpnum;
        $obuyinforows = array();
        $query->execute($sql);
        $k = 0;
        while ($row = $query->getRow()) {
            $obuyinforows[$k] = jieqi_funtoarray('jieqi_htmlstr', $row);
            $k++;
        }
