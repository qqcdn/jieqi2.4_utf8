...
switch ($_POST['act']) {
    case 'update':
        ...
        if (empty($errors)) {
            ...
            $draft->setVar('summary', $summary);
            if (isset($_POST['preface'])) {
                $draft->setVar('preface', $_POST['preface']);
            }
            if (isset($_POST['notice'])) {
                $draft->setVar('notice', $_POST['notice']);
            }
            if (isset($_POST['isbody'])) {
                $draft->setVar('isbody', intval($_POST['isbody']));
            }
            if (strlen($_POST['chaptercontent']) == 0 && 0 < count($oldattachary)) {
                $draft->setVar('isimage', 1);
            } else {
                $draft->setVar('isimage', 0);
            }
            $saleprice = -1;
