...
$jieqiTset['jieqi_contents_template'] = JIEQI_ROOT_PATH . '/templates/mysettlelog.html';
include_once JIEQI_ROOT_PATH . '/header.php';
jieqi_includedb();
$query = JieqiQueryHandler::getInstance('JieqiQueryHandler');
$jieqiPset = jieqi_get_pageset();
$sql = 'SELECT * FROM ' . jieqi_dbprefix('system_settlelog') . ' WHERE userid = ' . intval($_SESSION['jieqiUserId']) . ' ORDER BY logid DESC LIMIT ' . intval($jieqiPset['start']) . ', ' . intval($jieqiPset['rows']);
$query->execute($sql);
$settlerows = array();
$k = 0;
$growthmax = 0;
